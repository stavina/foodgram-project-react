    ...
    class Meta:
        ...
        constraints = (
            ...
            models.CheckConstraint(
                ...
                name="\n%(app_label)s_%(class)s_measurement_unit - пустое" "значение\n",
            ),
        )
...
class Tag(models.Model):
    ...
    slug = models.SlugField(
        max_length=200, unique=True, db_index=True, help_text="Уникальный слаг тэга"
    )
...
class Recipe(models.Model):
    ...
    tags = models.ManyToManyField(
        Tag, related_name="recipes", verbose_name="Тег", help_text="Выберите тэг"
    )
    cooking_time = models.PositiveSmallIntegerField(
        ...
        validators=[
            MinValueValidator(
                MIN_COOKING_TIME,
                "Минимальное время приготовления - " f"{MIN_COOKING_TIME} минута",
            ),
            MaxValueValidator(
                MAX_COOKING_TIME,
                "Максимальное время приготовления - " f"{MAX_COOKING_TIME} минуты",
            ),
        ],
    )
...
class Favorite(models.Model):
    ...
    date_added = models.DateTimeField(
        verbose_name="Дата добавления в избранное", auto_now_add=True, editable=False
    )
