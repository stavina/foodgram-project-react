...
class UserCreateSerializer(UserCreateSerializer):
    ...
    def validate(self, data):
        ...
        if not first_name:
            raise serializers.ValidationError({"first_name": "Имя - обязательное поле"})
        if not last_name:
            raise serializers.ValidationError(
...
class SetPasswordSerializer(serializers.Serializer):
    ...
    def update(self, instance, validated_data):
        ...
        if current_password == new_password:
            raise serializers.ValidationError(
                {"new_password": "Новый пароль должен отличаться от " "текущего пароля"}
            )
...
class RecipeCreateSerializer(RecipeSerializer):
    ...
    def validate(self, data):
        ...
        if not ingredients_amount:
            raise serializers.ValidationError("Укажите минимум 1 ингредиент.")
